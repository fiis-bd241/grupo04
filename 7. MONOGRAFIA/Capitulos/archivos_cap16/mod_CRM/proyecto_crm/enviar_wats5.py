...
whatsapp_api_url = 'https://graph.facebook.com/v19.0/327764460428670/messages'
access_token = 'EAAVsf3uqKBoBO94ZC8PdhGE5aYfcRy8ZAvlUxG7aZAlfQl93WNpxwvZASE5O56BZCMEwa7L3ZA55VKJFmNS4NhqxIeFTmOwHADFCgu3PbCZCfGCWcuuTqPy3ZCddZCm7QeRFtBxUMkCWzsDLs3R2FIt5d0rnTYN7EhpaJn6JtbpBpW8Cu6ZCy2vO9QdkfTlpEz1rAFh00w8LiADXIRfkAm0LIZD'
...
class WhatsAppApp:
    ...
    def load_sent_messages(self):
        ...
        try:
            connection = self.connect_db()
            cursor = connection.cursor()
            cursor.execute("SELECT mensaje_name, FechaHora FROM MensajeSend ORDER BY FechaHora DESC;")
            sent_messages = cursor.fetchall()
            for mensaje_name, fecha_hora in sent_messages:
                self.sent_messages_list.insert(tk.END, f"{mensaje_name} (Enviado: {fecha_hora})")
        except Exception as e:
    ...
    def save_sent_message(self, mensaje_name, recipients, content):
        ...
        try:
            ...
            cursor.execute(query, (mensaje_name, recipients, content, fecha_hora_actual, None))
            connection.commit()
            self.sent_messages_list.insert(tk.END, f"{mensaje_name} (Enviado: {fecha_hora_actual})")
        except Exception as e:
    ...
    def load_sent_message_details(self, event):
        ...
        selected_message = self.sent_messages_list.get(self.sent_messages_list.curselection()).split(" (Enviado:")[0]
        try:
            connection = self.connect_db()
            cursor = connection.cursor()

...
class EmailApp:
    ...
    def load_sent_emails(self):
        ...
        try:
            connection = self.connect_db()
            cursor = connection.cursor()
            cursor.execute("SELECT email_name, FechaHora FROM EmailSend ORDER BY FechaHora DESC;")
            sent_emails = cursor.fetchall()
            for email_name, fecha_hora in sent_emails:
                self.sent_emails_list.insert(tk.END, f"{email_name} (Enviado: {fecha_hora})")
        except Exception as e:
    ...
    def save_sent_email(self, email_name, recipients, content):
        ...
        try:
            connection = self.connect_db()
            cursor = connection.cursor()
            query = """INSERT INTO EmailSend (email_name, enviados, email_content, FechaHora) VALUES (%s, %s::text[], %s, %s)"""
            fecha_hora_actual = datetime.datetime.now()
            cursor.execute(query, (email_name, recipients, content, fecha_hora_actual))
            connection.commit()
            self.sent_emails_list.insert(tk.END, f"{email_name} (Enviado: {fecha_hora_actual})")
        except Exception as e:
    ...
    def load_sent_email_details(self, event):
        ...
        selected_email = self.sent_emails_list.get(self.sent_emails_list.curselection()).split(" (Enviado:")[0]
        try:
            connection = self.connect_db()
            cursor = connection.cursor()
